...
The purpose of this document is to outline of the methods used to contruct composite variables from the raw behaviors collected form the Human Intruder Task. The data include the **Stare, Familiar Food Offer, and Novel Object** epochs.
...
# this head output in a kabletable
kable(melted_corrnorm_selected_ffostare_v1, caption = "comm2") %>%
...
```{r creating the composite scores}
...
Composite scores were created by taking the value of each individual behavior from the output of community detection. 

```{r inserting demographic infomation}
