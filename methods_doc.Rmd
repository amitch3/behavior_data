...
The correlational values above were used to investigate the relationships amongst the behaviors. Specifically, how behaviors cluster together. To do this, an empirical approach was chosen that used community detection algorithm to determin which behaviors were clustering together. 


![The outputs from the various parameter changes I ran. The final groups were based on a consensus approach, which means I kept the groupings that appeared the most. One note, is that the smallest community was kept because the behaviors make sense as a group. Behaviors that share the same color are within the same group.](comm_detec_groups.PNG)
